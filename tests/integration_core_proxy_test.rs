...
fn test_core_monitor_proxy_functionality() {
    ...
    match result {
        Ok(responses) => {
            ...
            assert!(!responses.is_empty(), "Should receive responses from MCP server");

            // Find initialize response by ID (should be 1)
            let init_response = responses.iter().find(|r| r.get("id").and_then(|id| id.as_i64()) == Some(1));
            if let Some(init_resp) = init_response {
                assert_eq!(init_resp["jsonrpc"], "2.0", "Invalid JSON-RPC version");
                assert_eq!(init_resp["id"], 1, "Initialize response ID should match request");
                assert!(init_resp.get("result").is_some() || init_resp.get("error").is_some(),
            ...
            let tools_response = responses.iter().find(|r| r.get("id").and_then(|id| id.as_i64()) == Some(2));
            if let Some(tools_resp) = tools_response {
                assert_eq!(tools_resp["jsonrpc"], "2.0", "Invalid JSON-RPC version");
                assert_eq!(tools_resp["id"], 2, "Tools list response ID should match request");
            ...
            assert!(!log_content.trim().is_empty(), "Log file should not be empty");
            ...
            for (i, line) in log_lines.iter().enumerate() {
                if !line.trim().is_empty() {
                    ...
                    assert!(log_entry.get("timestamp").is_some(), "Log entry {} should have timestamp", i);
            ...
            assert!(command_entries > 0 || mcp_traffic_entries > 0,
                   "Log should contain either MCP traffic or command metadata");

            println!("✅ Log file validation passed: {} MCP entries, {} command entries", mcp_traffic_entries, command_entries);

            println!("✅ Core proxy test passed! Processed {} responses", responses.len());
        }
        Err(e) => {
...
fn test_core_proxy_with_invalid_server() {
    ...
    match child.try_wait() {
        Ok(Some(status)) => {
            ...
            assert!(!status.success(), "Proxy should fail when given invalid server command");
    ...
    if log_file.exists() {
        ...
        if !log_content.trim().is_empty() {
            ...
            for (i, line) in log_content.lines().enumerate() {
                ...
                let parsed: Result<serde_json::Value, _> = serde_json::from_str(line);
                assert!(parsed.is_ok(), "Log line {} should be valid JSON: {}", i, line);
                ...
                assert!(log_entry.get("timestamp").is_some(), "Log entry {} missing timestamp", i);
                assert!(log_entry.get("direction").is_some(), "Log entry {} missing direction", i);
                assert!(log_entry.get("content").is_some(), "Log entry {} missing content", i);

                // Verify direction is valid
                let direction = log_entry["direction"].as_str().unwrap();
                assert!(
                    direction == "request" || direction == "response",
                    "Log entry {} has invalid direction: {}", i, direction
                );
                ...
                assert!(
                    ...
                    "Log entry {} has invalid timestamp format: {}", i, timestamp
                );
            }
            ...
            if mcp_entries == 0 {
                ...
                println!("✅ Log format validation passed! Found {} MCP traffic entries", mcp_entries);
            }
        } else {
