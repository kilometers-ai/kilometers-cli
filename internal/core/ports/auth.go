...
type TokenProvider interface {
	...
	GetToken(ctx context.Context, request *domain.TokenRequest) (*domain.AuthToken, error)
...
type TokenCache interface {
	...
	GetToken(scope string) (*domain.AuthToken, error)

	// SetToken stores a token for the given scope
	SetToken(scope string, token *domain.AuthToken) error
...
type AuthManager interface {
	...
	GetValidToken(ctx context.Context, scope string) (*domain.AuthToken, error)

	// ForceRefresh forces a token refresh regardless of expiration
	ForceRefresh(ctx context.Context, scope string) (*domain.AuthToken, error)
...
type TokenRefreshStrategy interface {
	...
	ShouldRefresh(token *domain.AuthToken) bool
